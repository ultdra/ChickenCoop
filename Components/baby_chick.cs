...
public partial class baby_chick : CharacterBody2D
{
	...
	[Signal]
	public delegate void StartSleepingStateEventHandler();
    [Signal]
    public delegate void EndSleepingStateEventHandler();
    [Signal]
    public delegate void StartPlayStateEventHandler();
    [Signal]
    public delegate void EndPlayStateEventHandler();
    [Signal]
    public delegate void StartThinkingStateEventHandler();
    [Signal]
    public delegate void EndThinkingStateEventHandler();
    [Signal]
    public delegate void StartWanderStateEventHandler();
    [Signal]
    public delegate void EndWanderStateEventHandler();
    [Signal]
    public delegate void StartGrazingStateEventHandler();
    [Signal]
    public delegate void EndGrazingStateEventHandler();
...
#region Public state and stats manipulators
	public void ChangeState(BabyChickenStates newState)
    {
        ...
        {
            case (BabyChickenStates.Sleeping):
            case (BabyChickenStates.Relaxing):
                EmitSignal(SignalName.EndSleepingState);
                break;

            case (BabyChickenStates.Playing):
                EmitSignal(SignalName.EndPlayState);
                break;

            case (BabyChickenStates.Wandering):
                EmitSignal(SignalName.EndWanderState);
                break;

            case (BabyChickenStates.Thinking):
                EmitSignal(SignalName.EndThinkingState);
                break;

            case (BabyChickenStates.Grazing):
                EmitSignal(SignalName.EndGrazingState);
                break;
        }
        ...
        switch (currentChickenState)
        {
			case (BabyChickenStates.Sleeping):
            case (BabyChickenStates.Relaxing):
                EmitSignal(SignalName.StartSleepingState);
                break;

            case (BabyChickenStates.Playing):
                EmitSignal(SignalName.StartPlayState);
                break;

            case (BabyChickenStates.Wandering):
                EmitSignal(SignalName.StartWanderState);
                break;

            case (BabyChickenStates.Thinking):
                EmitSignal(SignalName.StartThinkingState);
                break;

            case (BabyChickenStates.Grazing):
                EmitSignal(SignalName.StartGrazingState);
                break;
        }
    }
